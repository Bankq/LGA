...
let rec expand_token_list = fun list ->
  let rec expand_token = fun token ->
    ...
    | _ -> [token] in
  let rec add_terminator = fun list ->
    match list with
    | OUTDENT :: (TERMINATOR :: (ELSE :: t)) -> OUTDENT :: (ELSE :: (add_terminator t))
    | [] -> []
    | _ -> (hd list) :: (add_terminator (tl list))
  in
  add_terminator (List.flatten (List.map expand_token list))
...
let ast_of_token_list myparser tokenlist = 
